;Prende LEDs usando retardo mas grandes.
;-----------------------------------------------------------------------------------------------------------------------------------
...
; SE ESTABILICE EL OSCILADOR INTERN2O, CUANDO PASA A 1 ARRANCA LA INICIALIZACION
	...
	banksel		ANSEL
	movlw		b'00000000'
	movwf		ANSEL			;Son todas digitales

	banksel		TRISA
	movlw		b'00000000'
	movwf		TRISA			;No se utilizan pines

	movlw		b'00000000'
	movwf		TRISB			;Solo se usa RB0 como salida

	banksel		OPTION_REG
	movlw		b'10000000'
	movwf		OPTION_REG		;Resistencias pull-up desactivadas
								;Oscilador interno
								;Preescaler 1:2
...
MAIN
		...
		BCF	 	PORTB,0			;Prende el LED
		movlw	.80
		movwf	veces			;80 veces 100ms son 8 segundos
		call	retardo
		BSF		PORTB,0			;Apaga el LED
		movlw	.40
		movwf	veces			;40 veces 100ms son 4 segundos
		call 	retardo
...
retardo
		...
		call	ret_100ms		;Ejecuta el retardo
		decfsz	veces,1			;Decrementa en 1 el contador que recuerda cuantas veces se llamo al retardo
